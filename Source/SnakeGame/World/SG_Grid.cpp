...
{
    if (!GetWorld() || !GetWorld()->GetLineBatcher(UWorld::ELineBatcherType::World)) return;

    for (uint32 i = 0; i < GridDim.height + 1; ++i)
    {
        const FVector StartLocation = GetActorLocation() + GetActorForwardVector() * CellSize * i;
        GetWorld()
            ->GetLineBatcher(UWorld::ELineBatcherType::World)
            ->DrawLine(StartLocation, StartLocation + GetActorRightVector() * WorldWidth, FLinearColor::Red, 0, 2.0f);
    ...
    for (uint32 i = 0; i < GridDim.width + 1; ++i)
    {
        ...
        GetWorld()
            ->GetLineBatcher(UWorld::ELineBatcherType::World)
            ->DrawLine(StartLocation, StartLocation + GetActorForwardVector() * WorldHeight, FColor::Red, false, -1.0f, 0, 2.0f);
