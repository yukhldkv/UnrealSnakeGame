...
{
public:
    [[nodiscard]] explicit TLinkedListIteratorBase(ContainerType* FirstLink) : CurrentLink(FirstLink) {}
    ...
    inline TLinkedListIteratorBase& operator++()
    {
        Next();
        return *this;
    }

    inline TLinkedListIteratorBase operator++(int)
    {
        auto Tmp = *this;
        Next();
        return Tmp;
    ...
    [[nodiscard]] UE_FORCEINLINE_HINT explicit operator bool() const { return CurrentLink != nullptr; }

    [[nodiscard]] UE_FORCEINLINE_HINT bool operator==(const TLinkedListIteratorBase& Rhs) const { return CurrentLink == Rhs.CurrentLink; }
    [[nodiscard]] UE_FORCEINLINE_HINT bool operator!=(const TLinkedListIteratorBase& Rhs) const { return CurrentLink != Rhs.CurrentLink; }
...
public:
    [[nodiscard]] explicit TLinkedListIterator(ContainerType* FirstLink) : Super(FirstLink) {}
...
public:
    [[nodiscard]] explicit TIntrusiveLinkedListIterator(ElementType* FirstLink) : Super(FirstLink) {}
...
public:
     ...
     */
    [[nodiscard]] TLinkedListBase() : NextLink(NULL), PrevLink(NULL) {}
     ...
     */
    inline void LinkBefore(ContainerType* Before)
    {
     ...
     */
    inline void LinkAfter(ContainerType* After)
    {
     ...
     */
    inline void LinkReplace(ContainerType* Replace)
    {
     ...
     */
    inline void LinkHead(ContainerType*& Head)
    {
        if (Head != NULL)
        {
     ...
     */
    [[nodiscard]] UE_FORCEINLINE_HINT bool IsLinked() { return PrevLink != nullptr; }

    [[nodiscard]] UE_FORCEINLINE_HINT ContainerType** GetPrevLink() const { return PrevLink; }

    [[nodiscard]] UE_FORCEINLINE_HINT ContainerType* GetNextLink() const { return NextLink; }

    [[nodiscard]] UE_FORCEINLINE_HINT ContainerType* Next() { return NextLink; }
...
private:
    ...
    [[nodiscard]] UE_FORCEINLINE_HINT friend TIterator begin(ContainerType& List) { return TIterator(&List); }
    [[nodiscard]] UE_FORCEINLINE_HINT friend TConstIterator begin(const ContainerType& List)
    {
        return TConstIterator(const_cast<ContainerType*>(&List));
    }
    [[nodiscard]] UE_FORCEINLINE_HINT friend TIterator end(ContainerType& List) { return TIterator(nullptr); }
    [[nodiscard]] UE_FORCEINLINE_HINT friend TConstIterator end(const ContainerType& List) { return TConstIterator(nullptr); }
};
...
public:
     ...
     */
    [[nodiscard]] explicit TLinkedList(const ElementType& InElement) : Super(), Element(InElement) {}

    // Accessors.
    [[nodiscard]] UE_FORCEINLINE_HINT ElementType* operator->() { return &Element; }
    [[nodiscard]] UE_FORCEINLINE_HINT const ElementType* operator->() const { return &Element; }
    [[nodiscard]] UE_FORCEINLINE_HINT ElementType& operator*() { return Element; }
    [[nodiscard]] UE_FORCEINLINE_HINT const ElementType& operator*() const { return Element; }
...
public:
    /** Default constructor (empty list). */
    [[nodiscard]] TIntrusiveLinkedList() : Super() {}
};
...
{
public:
    [[nodiscard]] explicit TDoubleLinkedListIterator(NodeType* StartingNode) : CurrentNode(StartingNode) {}

    /** conversion to "bool" returning true if the iterator is valid. */
    [[nodiscard]] UE_FORCEINLINE_HINT explicit operator bool() const { return CurrentNode != nullptr; }
...
public:
    ...
    public:
        ...
        /** Constructor */
        [[nodiscard]] TDoubleLinkedListNode(const ElementType& InValue) : Value(InValue), NextNode(nullptr), PrevNode(nullptr) {}

        [[nodiscard]] const ElementType& GetValue() const { return Value; }

        [[nodiscard]] ElementType& GetValue() { return Value; }

        [[nodiscard]] TDoubleLinkedListNode* GetNextNode() { return NextNode; }

        [[nodiscard]] const TDoubleLinkedListNode* GetNextNode() const { return NextNode; }

        [[nodiscard]] TDoubleLinkedListNode* GetPrevNode() { return PrevNode; }

        [[nodiscard]] const TDoubleLinkedListNode* GetPrevNode() const { return PrevNode; }
    ...
    /** Constructors. */
    [[nodiscard]] TDoubleLinkedList() : HeadNode(nullptr), TailNode(nullptr), ListSize(0) {}
    ...
    [[nodiscard]] TDoubleLinkedList(TDoubleLinkedList&& Other) : TDoubleLinkedList()
    {
     ...
     * @see GetTail
     */
    [[nodiscard]] TDoubleLinkedListNode* GetHead() const { return HeadNode; }
     ...
     * @see GetHead
     */
    [[nodiscard]] TDoubleLinkedListNode* GetTail() const { return TailNode; }
    ...
    [[nodiscard]] bool Contains(const ElementType& InElement) { return (FindNode(InElement) != nullptr); }
     ...
     * @see Num
     */
    [[nodiscard]] bool IsEmpty() const { return ListSize == 0; }
     ...
     * @return Item count.
     */
    [[nodiscard]] int32 Num() const { return ListSize; }
